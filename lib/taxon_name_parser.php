...
class Parser 
{
    ...
    {    
		...
		// Try and parse name
		if (preg_match($p, $result->scientificName->normalised, $m, PREG_OFFSET_CAPTURE))
		{
			//print_r($m);
			
			$result->start = $m[0][1];
			$result->end = mb_strlen($m[0][0]);
			...
			if ($m['genus'][0] != '')
			{
				...
				$details->genus->epitheton = $m['genus'][0];	
				
				if (($m['authorship'][0] != '') && ($m['infragenus'][0] == '') && ($m['species'][0] == ''))
				{
			...
			if ($m['infragenus'][0] != '')
			{
				...
				$details->infragenus->epitheton = $m['infragenus'][0];
				
				$details->infragenus->epitheton = preg_replace('/^\(/', '', $details->infragenus->epitheton);
				$details->infragenus->epitheton = preg_replace('/\)$/', '', $details->infragenus->epitheton);
				
				if (($m['authorship'][0] != '') && ($m['species'][0] == ''))
				{
			...
			if ($m['species'][0] != '')
			{
				...
				$details->species->epitheton = $m['species'][0];
				
				if (($m['authorship'][0] != '') && ($m['infraspecies'][0] == ''))
				{
			...
			if ($m['infraspecies'][0] != '')
			{
				...
				$details->infraspecies->epitheton = $m['infraspecies'][0];
				if ($m['rank'][0] != '')
				{
					$details->infraspecies->rank = $m['rank'][0];
				}
	...
	function authorship(&$name, $m)
	{
		if ($m['authorship'][0] != '')
		{
			$name->authorship = $m['authorship'][0];
			$name->authorship = preg_replace('/^\s+/u', '', $name->authorship);
			
			if ($m['basionymAuthorTeam'][0] != '')
			{
				$name->basionymAuthorTeam = new stdclass;
				$name->basionymAuthorTeam->authorTeam = $m['basionymAuthorTeam'][0];
				if (isset($m['basionymYear'][0]) && ($m['basionymYear'][0] != ''))
				{
					$name->basionymAuthorTeam->year = $m['basionymYear'][0];
				}
			}
			if (isset($m['authorTeam'][0]) && ($m['authorTeam'][0] != ''))
			{
				if ($m['basionymAuthorTeam'][0] == '')
				{
					$name->basionymAuthorTeam = new stdclass;
					$name->basionymAuthorTeam->authorTeam = $m['authorTeam'][0];
					if (isset($m['year'][0]) && ($m['year'][0] != ''))
					{
						$name->basionymAuthorTeam->year = $m['year'][0];						
					}
				}
				else
				{
					$name->combinationAuthorTeam = new stdclass;
					$name->combinationAuthorTeam->authorTeam = $m['authorTeam'][0];
					if (isset($m['year'][0]) && ($m['year'][0] != ''))
					{
						$name->combinationAuthorTeam->year = $m['year'][0];						
					}
				}
...
if (1)
{
	...
	//$n = 'Tospitis fusipennana Walker, 1863, List Specimens lepid. Insects Colln Br. Mus., 28: 431.';
	//$n = 'Tospitis fusipennana Walker; Kirby, 1892, Synonymic Cat. lepid. Heterocera 1 (1): 365.';
	//$n = 'Byrsia aurantiaca javana Rothschild & Jordan, 1901, Novit. zool., 8: 422.';
	$n = '"Bacterium aquatilis" (sic) (Frankland and Frankland 1889) Chester 1897';
	$n = 'Pitane biplaga Felder, 1875, Reise öst. Fregatte Novara, 139: 5.';
	$n = 'Caprimima occidentalis Rothschild & Jordan, 1901, Novit. zool., 8: 420.';
	$n = 'Utriculifera [sic] macroplaga Hampson, 1900, Cat. Lepid. Phalaenae Br. Mus., 2: 290.';
	$n = 'Mellona blanda van Eecke, 1927, Zool. Meded. Leiden, 10: 94.';
	
	
	$r = $pp->parse($n);
	
	// what is left after the name has been parsed?
	if ($r->scientificName->parsed)
	{
		$r->tail = mb_substr($n, $r->end);
	}
